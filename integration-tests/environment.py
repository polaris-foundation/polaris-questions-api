...
from helpers.jwt import decode_jwt, get_patient_token, get_system_token
...
def before_scenario(context: Context, scenario: Scenario) -> None:
    ...
    if not hasattr(context, "patient_jwt"):
        context.patient_jwt = get_patient_token()
        context.patient_uuid = decode_jwt(context.patient_jwt)["metadata"]["patient_id"]

    # placeholder to keep track of created objects in case we need them
    for item in ["question_uuid", "survey_uuid"]:
        if not hasattr(context, item):
            setattr(context, item, [])
